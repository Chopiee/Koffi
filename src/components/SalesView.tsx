...
import { AddSalesTransactionModal } from './AddSalesTransactionModal';
import { CreateSalesPage, CreateSalesInitialData } from './CreateSalesPage';
...
type SortField = 'id' | 'customer' | 'status' | 'date' | 'deadline' | 'invoice' | 'remaining' | 'amount';
...
export function SalesView({
  ...
  // Default empty sales transactions
  ...
  const [createInitialData, setCreateInitialData] = useState<CreateSalesInitialData>({
    customer: 'Booking',
    invoiceNo: '',
    ...
    return transactions
      .filter((tx) => {
        ...
        if (selectedCustomerFilter !== 'All') {
          const cust = tx.connection || tx.supplier || '';
          if (cust.toLowerCase() !== selectedCustomerFilter.toLowerCase()) return false;
        }
  ...
  // Format readable date exactly like PurchaseView (e.g. June 10, 2023)
  ...
  // Render Customer Logo / Avatar
  const renderCustomerCell = (tx: PurchaseTransaction) => {
    const connName = tx.connection || tx.supplier || 'N/A';

    if (connName === 'Booking') {
      return (
        <div className="flex items-center gap-2.5 max-w-[160px] md:max-w-[200px]">
          ...
          <span className="font-medium text-neutral-900 text-[12.5px] truncate" title="Booking">Booking</span>
        </div>
      );
    }
    ...
    if (connName === 'Airbnb') {
      return (
        <div className="flex items-center gap-2.5 max-w-[160px] md:max-w-[200px]">
          ...
          <span className="font-medium text-neutral-900 text-[12.5px] truncate" title="Airbnb">Airbnb</span>
        </div>
      );
    }
    ...
    if (connName === 'Guesty') {
      return (
        <div className="flex items-center gap-2.5 max-w-[160px] md:max-w-[200px]">
          ...
          <span className="font-medium text-neutral-900 text-[12.5px] truncate" title="Guesty">Guesty</span>
        </div>
      );
    }

    if (connName === 'N/A' || connName === 'Walk-in Guest') {
      return (
        <div className="flex items-center gap-2.5 max-w-[160px] md:max-w-[200px]">
          <div className="w-6 h-6 rounded-full bg-neutral-200 text-neutral-600 flex items-center justify-center shrink-0 shadow-2xs">
            <Building2 size={12} />
          </div>
          <span className="font-medium text-neutral-700 text-[12.5px] truncate" title={connName}>{connName}</span>
        </div>
      );
    }
    ...
    return (
      <div className="flex items-center gap-2.5 max-w-[160px] md:max-w-[200px]">
        ...
        <span className="font-medium text-neutral-900 text-[12.5px] truncate" title={connName}>
          {connName}
        </span>
      </div>
    );
    ...
    setInternalTransactions((prev) =>
      prev.map((t) =>
        selectedIds.includes(t.id)
          ? { ...t, status: newStatus, tag: newStatus }
          : t
      )
    );
    setSelectedIds([]);
  ...
  // Save new Sales Transaction
  const handleSaveSalesTransaction = (newTxData: Omit<PurchaseTransaction, 'id' | 'createdAt'>) => {
  ...
  if (viewMode === 'create-details') {
    return (
      <CreateSalesPage
        ...
        onSave={handleSaveSalesTransaction}
      />
    );
  }
  ...
  return (
    <div className="w-full max-w-full pt-4 md:pt-6 pb-20 px-4 md:px-6 font-sans text-neutral-900">
      ...
      <div ref={filterDropdownRef} className="flex flex-wrap items-center justify-between gap-3 mb-4 text-[12px]">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative inline-block">
            ...
            {activeFilterDropdown === 'status' && (
              <div className="absolute top-[calc(100%+4px)] left-0 z-40 bg-white rounded-lg shadow-lg border border-neutral-200/80 p-2 w-48 animate-in fade-in zoom-in-95 duration-100">
                <div className="text-[11px] text-neutral-400 font-medium px-2 py-1">
                  Select status
                </div>
          ...
          <div className="relative inline-block">
            <button
              id="filter-pill-customer"
              onClick={() =>
                setActiveFilterDropdown(activeFilterDropdown === 'customer' ? null : 'customer')
              }
              className={`bg-white hover:bg-neutral-50 border rounded-md px-2.5 py-1 flex items-center gap-1.5 transition-colors cursor-pointer text-neutral-800 shadow-2xs font-normal text-[12px] ${
            ...
            >
              ...
              <span>{selectedCustomerFilter !== 'All' ? selectedCustomerFilter : 'Customer'}</span>
              <ChevronDown size={11} className="text-neutral-400 ml-0.5" />
            </button>

            {activeFilterDropdown === 'customer' && (
              <div className="absolute top-[calc(100%+4px)] left-0 z-40 bg-white rounded-lg shadow-lg border border-neutral-200/80 p-2 w-48 animate-in fade-in zoom-in-95 duration-100 max-h-60 overflow-y-auto">
                <div className="text-[11px] text-neutral-400 font-medium px-2 py-1">
                  Select customer
                </div>
                {['All', 'Booking', 'Airbnb', 'Guesty', 'Walk-in Guest', 'Corporate Client', 'Direct Customer', 'Sunset Resort', 'N/A'].map((dist) => (
                  <button
                    key={dist}
                    onClick={() => {
                      setSelectedCustomerFilter(dist);
                      setActiveFilterDropdown(null);
                    }}
                    className={`w-full px-2 py-1 rounded text-left text-[12px] flex items-center justify-between cursor-pointer transition-colors ${
                      selectedCustomerFilter === dist
                        ? 'bg-neutral-100 font-medium text-neutral-900'
                        : 'text-neutral-700 hover:bg-neutral-50'
                    }`}
                  >
                    <span>{dist}</span>
                    {selectedCustomerFilter === dist && <Check size={12} className="text-neutral-900" />}
                  </button>
                ))}
              </div>
            )}
        ...
        {/* Right Controls - Add Sales Button */}
        <div className="flex items-center gap-2">
          {/* Add Sales Button */}
          <button
            id="btn-add-sales-top"
      ...
      {selectedIds.length > 0 && (
        <div className="mb-4 p-2 px-3.5 rounded-lg bg-neutral-900 text-white flex items-center justify-between text-[12px] shadow-sm animate-in fade-in duration-150">
          <div className="flex items-center gap-2">
      ...
      {/* 4. Table Card exact 1:1 like Purchase */}
          ...
          <tbody className="divide-y divide-neutral-100 font-sans text-[12.5px] text-neutral-800">
            {filteredTransactions.length === 0 ? (
              ...
              paginatedTransactions.map((tx) => {
                ...
                const formattedId = (tx.id.startsWith('tx-') || (tx.id.startsWith('sl-') && tx.id.length > 8))
                  ? `#${tx.id.replace(/^(tx-|sl-)/, '').slice(0, 6).toUpperCase()}`
                  : `#${tx.id.toUpperCase()}`;
