...
import { AddCostTransactionModal, CreateCostInitialData } from './AddCostTransactionModal';
...
type SortField = 'id' | 'vendor' | 'status' | 'date' | 'deadline' | 'invoice' | 'amount';
...
export function CostView({
  ...
  // Default empty cost transactions
  ...
  const [createInitialData, setCreateInitialData] = useState<CreateCostInitialData>({
    vendor: 'Utilities & Electricity',
    invoiceNo: '',
  ...
  const handleDelete = (id: string) => {
    if (propOnDeleteTransaction) {
      ...
      setInternalTransactions((prev) => prev.filter((t) => t.id !== id));
    }
  ...
  const handleBulkSetStatus = (status: string) => {
    selectedIds.forEach((id) => {
      ...
      if (tx) {
        const updated = { ...tx, status, tag: status };
        if (propOnUpdateTransaction) {
          propOnUpdateTransaction(updated);
        } else {
          setInternalTransactions((prev) =>
    ...
    return transactions
      .filter((tx) => {
        ...
        if (selectedVendorFilter !== 'All') {
          const vendor = tx.supplier || tx.connection || tx.category || '';
          if (vendor.toLowerCase() !== selectedVendorFilter.toLowerCase()) return false;
        }
        ...
        if (scannedBarcode.trim()) {
          ...
          const matchCategory = (tx.category || '').toLowerCase().includes(code);
          if (!matchInvoice && !matchId && !matchSupplier && !matchDesc && !matchNotes && !matchCategory) return false;
        }
      ...
      .sort((a, b) => {
        ...
        if (sortField === 'id') {
          ...
          valA = a.supplier || a.connection || a.category || '';
          valB = b.supplier || b.connection || b.category || '';
          ...
          valA = a.notes || a.description || '';
          valB = b.notes || b.description || '';
  ...
  const renderPaymentMethodBadge = (paymentText?: string) => {
    const text = paymentText || 'Bank Transfer';

    return (
      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md text-[12.5px] font-normal border transition-colors bg-[#F3F4F6] text-neutral-700 border-neutral-200/70">
        <CreditCard size={12} className="shrink-0 text-neutral-400" />
        <span className="whitespace-nowrap">{text}</span>
      </div>
    );
  };

  // Render Vendor Avatar / Logo
  const renderVendorCell = (tx: PurchaseTransaction) => {
    const vendorName = tx.supplier || tx.connection || tx.category || 'N/A';
    if (vendorName === 'N/A') {
      return (
    ...
    return (
      <div className="flex items-center gap-2.5">
        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-slate-700 to-neutral-900 text-white flex items-center justify-center font-bold text-[11px] shrink-0 shadow-xs">
          {initial}
        </div>
  ...
  if (viewMode === 'create-details') {
    const initialDataForCreate: CreatePurchaseInitialData = {
      distributor: createInitialData.vendor,
      invoiceNo: createInitialData.invoiceNo,
      date: createInitialData.date,
      dueDate: createInitialData.dueDate,
    };
    ...
    <div className="w-full max-w-full pt-4 md:pt-6 pb-20 px-4 md:px-6 font-sans text-neutral-900">
      ...
      <div ref={filterDropdownRef} className="flex flex-wrap items-center justify-between gap-3 mb-4 text-[12px]">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative inline-block">
            <button
              id="filter-pill-vendor"
              onClick={() =>
                setActiveFilterDropdown(activeFilterDropdown === 'vendor' ? null : 'vendor')
              }
              className={`bg-white hover:bg-neutral-50 border rounded-md px-2.5 py-1 flex items-center gap-1.5 transition-colors cursor-pointer text-neutral-800 shadow-2xs font-normal text-[12px] ${
            ...
            >
              ...
              <span>{selectedVendorFilter !== 'All' ? selectedVendorFilter : 'Recipient'}</span>
              <ChevronDown size={11} className="text-neutral-400 ml-0.5" />
            </button>

            {activeFilterDropdown === 'vendor' && (
              <div className="absolute top-[calc(100%+4px)] left-0 z-40 bg-white rounded-lg shadow-lg border border-neutral-200/80 p-1.5 w-56 max-h-56 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                ...
                {[
                  ...
                  >
                    ...
                    {selectedVendorFilter === v && <Check size={12} className="text-neutral-900 shrink-0" />}
                  </button>
                ))}
              </div>
            )}
        ...
        {/* Right Controls - Add Cost Button */}
        <div className="flex items-center gap-2">
          <button
            id="btn-add-cost-top"
            onClick={() => {
              setCreateInitialData({
                vendor: 'Utilities & Electricity',
                invoiceNo: `CST-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
                date: new Date().toISOString().split('T')[0],
                dueDate: '',
              });
      ...
      {/* 4. Selection Batch Action Bar */}
      {selectedIds.length > 0 && (
        <div className="mb-4 p-2.5 px-4 rounded-xl bg-neutral-900 text-white flex items-center justify-between text-[13px] shadow-sm animate-in fade-in duration-150">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-emerald-700 hover:bg-emerald-600 text-white transition-colors cursor-pointer flex items-center gap-1 text-[12px]"
            >
              <Check size={12} />
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-amber-700 hover:bg-amber-600 text-white transition-colors cursor-pointer flex items-center gap-1 text-[12px]"
            >
              <span>Mark as Partial</span>
            </button>
            <button
              onClick={() => handleBulkSetStatus('Unpaid')}
              className="px-2.5 py-1 rounded-md bg-neutral-800 hover:bg-neutral-700 text-neutral-200 transition-colors cursor-pointer flex items-center gap-1 text-[12px]"
            >
              <span>Mark as Unpaid</span>
            </button>
            <button
              onClick={handleBulkDelete}
              className="px-2.5 py-1 rounded-md bg-rose-600/80 hover:bg-rose-600 text-white transition-colors cursor-pointer flex items-center gap-1 text-[12px]"
            >
              <Trash2 size={12} />
              <span>Delete</span>
            </button>
      ...
      {/* 5. Main Table matching Purchase & Sales: ID, Vendor / Category, Status, Date, Deadline, Invoice, Amount, Action */}
        ...
        <table className="w-full text-left border-collapse min-w-[850px]">
          ...
          <tbody className="divide-y divide-neutral-100 font-sans text-[12.5px] text-neutral-800">
            {filteredTransactions.length === 0 ? (
              <tr>
                <td colSpan={8} className="py-12 text-center text-neutral-400 text-[13px]">
                  No cost records found matching the current filters.
                </td>
              </tr>
            ) : (
              paginatedTransactions.map((tx) => {
                const statusValue = tx.status || tx.tag || 'Paid';
                const formattedId = tx.id.startsWith('cst-')
                  ? `#CST-${tx.id.slice(4, 9).toUpperCase()}`
                  : `#${tx.id.toUpperCase()}`;
                  ...
                  >
                    ...
                    <td className="py-2 px-4 whitespace-nowrap text-[12.5px] max-w-[160px]">
                      <div className="flex items-center gap-1.5 text-neutral-700 max-w-[150px]">
                        ...
                        <span className="font-mono text-[12.5px] text-neutral-800 hover:text-neutral-950 underline decoration-neutral-300 underline-offset-2 truncate" title={tx.invoiceNo || `CST-${tx.id.toUpperCase()}`}>
                          {tx.invoiceNo || `CST-${tx.id.toUpperCase()}`}
                        </span>
                      </div>
                    </td>
                    ...
                    {/* Reference (Catatan) */}
                    <td className="py-2 px-4 whitespace-nowrap text-[12.5px] max-w-[200px] md:max-w-[250px]">
                      <span
                        className="text-neutral-700 font-normal truncate block"
      ...
      {/* 6. Add Cost Transaction Modal (Step 1 Centered Overlay) */}
      <AddCostTransactionModal
